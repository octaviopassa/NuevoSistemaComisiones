...
var myapp_config = {
  ...
  /*
	...
	detected a className is inserted to $.root_ and the variable thisDevice 
  ...
  navInitalized: "js-nav-built", //nav finished className
  ...
  /*
	...
	This uses the fontawesome css className
	*/
      ...
      if (!self.hasClass(opts.initClass)) {
        ...
        self.find("li a").on("mousedown", function (e) {
          if ($(this).parent().find("ul").length !== 0) {
            ...
            /**
             * Add active className to open element
             **/
            if (
  ...
  function Plugin(element, options) {
    ...
    function init() {
      ...
      $el.wrap(
        '<div id="' +
          options.wrapperId +
          '" className="nav-menu-wrapper d-flex flex-grow-1 width-0 overflow-hidden"></div>'
      );
      ...
      $("#" + options.wrapperId).before(
        '<a href="#" id="' +
          options.wrapperId +
          '-left-btn" className="d-flex align-items-center justify-content-center width-4 btn mt-1 mb-1 mr-2 ml-1 p-0 fs-xxl text-primary"><i className="fal fa-angle-left"></i></a>'
      );
      $("#" + options.wrapperId).after(
        '<a href="#" id="' +
          options.wrapperId +
          '-right-btn" className="d-flex align-items-center justify-content-center width-4 btn mt-1 mb-1 mr-1 ml-2 p-0 fs-xxl text-primary"><i className="fal fa-angle-right"></i></a>'
      );
...
var initApp = (function (app) {
  ...
  app.listFilter = function (list, input, anchor) {
    /* add className to filter hide/show */
    if (anchor) {
  ...
  app.removeSettings = function (classToRemove) {
    if (classToRemove != undefined) {
      // find and remove the specific className name from existing classes
      ...
      console.log("ERROR: You must specify the className you need to remove");
    }
  ...
  /*app.indicateSelections = function () {
		...
		var classNames = initApp.getSettings()
			.split(' ')
			.map(function(c) {
				return '[data-className="' +  c + '"].js-indicateSelections';
			})
			.join(',');

		$('[data-className].active.js-indicateSelections').removeClass('active');
  ...
  app.checkNavigationOrientation = function () {
    ...
    switch (true) {
      ...
      /* fires when users switch off nav-function-top className */
      case !myapp_config.root_.hasClass("nav-function-top") &&
        $("#js-nav-menu-wrapper").length == true:
        /* destroy horizontal nav */
        myapp_config.navHooks.menuSlider("destroy");
  ...
  app.appForms = function (parentClass, focusClass, disabledClass) {
    ...
    /* if input has 'some value' add className .has-length to .form-group */
  ...
  app.domReadyMisc = function () {
    ...
    /* Give modal backdrop an extra className to make it customizable */
    ...
    myapp_config.root_.on("click touchend", "[data-action]", function (e) {
      ...
      switch (true) {
        /**
         * toggle trigger
         * Usage 1 (body): <a href="#" data-action="toggle" data-className="add-this-className-to-body">...</a>
         * Usage 2 (target): <a href="#" data-action="toggle" data-className="add-this-className-to-target" data-target="target">...</a>
         **/
        case actiontype === "toggle":
          ...
          /* trigger className change */
          $(target).toggleClass(dataClass);

          /* this allows us to add active className for dropdown toggle components */
        ...
        /**
         * toggle swap trigger
         * Usage (target): <a href="#" data-action="toggle-swap" data-className="root-text" data-target="html">...</a>
         **/
        case actiontype === "toggle-swap":
          ...
          if (dataClass.startsWith("root-text")) {
            $("[data-className]").removeClass("active");
            $('[data-className="' + dataClass + '"]').addClass("active");
          }
         ...
         * Usage (target): <a href="#" data-action="toggle-swap" data-replaceclass="classesToReplace" data-className="replaceWithClass" data-target="body">...</a>
         **/
          ...
          if (typeof bootbox != "undefined") {
            ...
            bootbox.confirm({
              title:
                "<i className='" +
                myapp_config.appIconPrefix +
                " fa-times-circle text-danger mr-2'></i> Do you wish to delete panel <span className='fw-500'>&nbsp;'" +
                selectedPanel
                  .children(".panel-hdr")
                  .children("h2")
                  .text()
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /**
   * a. check for mobile view width and add className .mobile-view-activated
   **/
  initApp.mobileCheckActivation();
  ...
  /**
   * e. run app forms className detectors [parentClass,focusClass,disabledClass]
   **/
